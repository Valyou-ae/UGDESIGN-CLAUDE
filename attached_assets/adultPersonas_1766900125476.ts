/**
 * ADULT PERSONAS (Ages 30-50) - Complete 112 personas
...
function generateAdultPersona(ethnicity: string, sex: 'Female' | 'Male', size: string, sizeIndex: number): Persona {
  ...
  return {
    ...
    fullDescription: `${name} is a ${35 + sizeIndex * 2}-year-old adult ${sex.toLowerCase()} of ${ethnicity} heritage, standing ${specs.height} tall and weighing approximately ${specs.weight}. ${sex === 'Female' ? 'She' : 'He'} has a ${specs.build} with mature proportions. ${sex === 'Female' ? 'Her' : 'His'} face has defined features, wise ${eyeColor.toLowerCase()} eyes, and a confident professional expression. ${sex === 'Female' ? 'Her' : 'His'} ${hairColor.toLowerCase()} hair is ${hairStyle.toLowerCase()}.`
  };
}

export const ADULT_FEMALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as any;
export const ADULT_MALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as any;

ETHNICITIES.forEach(ethnicity => {
  ADULT_FEMALE_PERSONAS[ethnicity] = {} as any;
  ADULT_MALE_PERSONAS[ethnicity] = {} as any;
...
console.log(`Total Adult Personas: ${getAllAdultPersonas().length}`); // 112
