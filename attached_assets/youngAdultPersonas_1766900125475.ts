/**
 * YOUNG ADULT PERSONAS (Ages 18-29) - Complete 112 personas
...
function generateYoungAdultPersona(ethnicity: string, sex: 'Female' | 'Male', size: string, sizeIndex: number): Persona {
  ...
  return {
    ...
    fullDescription: \`\${name} is a \${20 + sizeIndex}-year-old young adult \${sex.toLowerCase()} of \${ethnicity} heritage, standing \${specs.height} tall and weighing approximately \${specs.weight}. \${sex === 'Female' ? 'She' : 'He'} has a \${specs.build} with youthful proportions. \${sex === 'Female' ? 'Her' : 'His'} face has clear skin, bright \${eyeColor.toLowerCase()} eyes, and a fresh youthful appearance. \${sex === 'Female' ? 'Her' : 'His'} \${hairColor.toLowerCase()} hair is \${hairStyle.toLowerCase()}, framing \${sex === 'Female' ? 'her' : 'his'} \${skinTone.toLowerCase()}.\`
  };
}

export const YOUNG_ADULT_FEMALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as any;
export const YOUNG_ADULT_MALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as any;

ETHNICITIES.forEach(ethnicity => {
  YOUNG_ADULT_FEMALE_PERSONAS[ethnicity] = {} as any;
  YOUNG_ADULT_MALE_PERSONAS[ethnicity] = {} as any;
...
console.log(\`Total Young Adult Personas: \${getAllYoungAdultPersonas().length}\`); // 112
