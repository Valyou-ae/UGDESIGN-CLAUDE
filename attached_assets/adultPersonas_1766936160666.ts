...
const HAIR_COLORS: Record<Ethnicity, string> = {
  'White': 'Light brown with subtle gray',
  'Black': 'Black',
  'Hispanic': 'Dark brown',
  'Asian': 'Black',
  'Indian': 'Black',
...
function generateAdultPersona(
  ...
  const age = 25 + (sizeIndex * 3); // Ages 25-40 across sizes
...
export const adultPersonas: Persona[] = [];
...
ETHNICITIES.forEach(ethnicity => {
  ['Female', 'Male'].forEach(sex => {
    SIZES.forEach((size, index) => {
      adultPersonas.push(
        generateAdultPersona(ethnicity, sex as 'Female' | 'Male', size, index)
      );
    });
  });
});
...
export function getAdultPersona(
  ...
  return adultPersonas.find(
    p => p.ethnicity === ethnicity && p.sex === sex && p.size === size
  );
}

export function getAdultPersonasByEthnicity(ethnicity: Ethnicity): Persona[] {
  return adultPersonas.filter(p => p.ethnicity === ethnicity);
}

export function getAdultPersonasBySex(sex: 'Female' | 'Male'): Persona[] {
  return adultPersonas.filter(p => p.sex === sex);
}

export function getAdultPersonasBySize(size: Size): Persona[] {
  return adultPersonas.filter(p => p.size === size);
}

// Total: 96 Adult Personas (8 ethnicities × 6 sizes × 2 sexes)
export default adultPersonas;
