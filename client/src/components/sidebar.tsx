...
export function Sidebar({ className }: SidebarProps) {
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 h-[60px] bg-background border-t border-border z-50 flex items-center justify-around px-2 pb-safe">
      ...
      <Link href="/image-gen">
        <div className="flex flex-col items-center justify-center -mt-6 cursor-pointer">
           <div className="h-14 w-14 rounded-full bg-gradient-to-br from-[#f8991c] to-[#B8860B] flex items-center justify-center shadow-lg shadow-[#f8991c]/30 border-4 border-background">
  ...
  return (
    <>
      ...
      <aside 
        className={cn(
          "relative h-screen border-r border-charcoal-200 dark:border-white/10 bg-white dark:bg-black/60 backdrop-blur-xl transition-all duration-300 ease-in-out flex-col z-40 hidden md:flex",
              ...
              <Link key={item.name} href={item.href}>
                <div
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? "text-black dark:text-white bg-charcoal-100 dark:bg-white/15" 
                      : "text-charcoal-500 dark:text-white/50 hover:bg-charcoal-100 dark:hover:bg-white/10 hover:text-black dark:hover:text-white"
                  )}
                >
                  <item.icon 
                    className={cn(
                      "h-7 w-7 flex-shrink-0 transition-all duration-200 group-hover:scale-110", 
                      isActive ? "text-black dark:text-white" : "text-charcoal-500 dark:text-white/50 group-hover:text-black dark:group-hover:text-white"
                    )} 
                  />
                  <span className={cn(
                      ...
                      >
                        ...
                        {item.badge && (
                          <span className={cn(
                            ...
                            isDiscover 
                              ? "bg-gradient-to-r from-[#f8991c] to-[#B8860B] text-white"
          ...
          <>
              ...
              {isLoading ? (
                collapsed ? (
                  ...
                  <div className="flex items-center gap-3 px-3.5 py-3">
                    <div className="h-7 w-7 rounded-full bg-charcoal-700/50 animate-pulse" />
                ...
                <div className={cn(
                  ...
                  {user.profileImageUrl ? (
                    <img 
                      ...
                      className="h-5 w-5 rounded-full object-cover border border-charcoal-300 dark:border-white/30 group-hover:border-charcoal-500 dark:group-hover:border-white/60 transition-all"
                      data-testid="img-user-avatar-sidebar"
                    />
                  ) : (
                    <div className="h-5 w-5 rounded-full bg-charcoal-200 dark:bg-charcoal-700 flex items-center justify-center border border-charcoal-300 dark:border-white/30 group-hover:border-charcoal-500 dark:group-hover:border-white/60 transition-all">
                      <span className="text-[8px] font-semibold text-charcoal-600 dark:text-white/80">
                  ...
                  <span className={cn(
                    ...
                    location === "/profile" ? "text-black dark:text-white" : "text-charcoal-500 dark:text-white/50 group-hover:text-black dark:group-hover:text-white"
                  )}>
                    Profile
                  </span>
                </div>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link href="/billing">
                    <div className="flex flex-col items-center justify-center py-2 px-2 mx-auto w-[52px] cursor-pointer">
                      ...
                      <span className="text-[9px] font-medium text-charcoal-500 dark:text-white/50 mt-1">{credits}</span>
                    </div>
                  </Link>
                </TooltipTrigger>
                ...
                <Link href="/billing">
                  <div className="flex items-center gap-3 rounded-lg font-medium text-charcoal-500 dark:text-white/50 hover:bg-charcoal-100 dark:hover:bg-white/10 hover:text-black dark:hover:text-white transition-all cursor-pointer group select-none px-3.5 py-3 text-sm">
      ...
      <div className={cn("mt-auto pb-6", collapsed ? "px-3 flex flex-col items-center pt-2 space-y-2" : "px-3 pt-4 space-y-2")}>
        ...
        {!user && !isLoading && (
          collapsed ? (
            <div
              onClick={() => openLoginPopup()}
              className="flex flex-col items-center justify-center gap-1 py-2 px-2 rounded-lg font-medium transition-all cursor-pointer group select-none mx-auto w-[52px] text-charcoal-500 dark:text-white/50 hover:bg-charcoal-100 dark:hover:bg-white/10 hover:text-black dark:hover:text-white"
              data-testid="button-login-sidebar"
            >
              <LogIn className="h-5 w-5 flex-shrink-0 transition-all duration-200 group-hover:scale-110 text-charcoal-500 dark:text-white/50 group-hover:text-black dark:group-hover:text-white" />
              <span className="text-[9px] font-medium text-charcoal-500 dark:text-white/50 group-hover:text-black dark:group-hover:text-white">
                Login
              </span>
            </div>
          ) : (
            <div
              onClick={() => openLoginPopup()}
              className="flex items-center gap-3 rounded-lg font-medium transition-all cursor-pointer group select-none px-3.5 py-3 text-sm text-charcoal-500 dark:text-white/50 hover:bg-charcoal-100 dark:hover:bg-white/10 hover:text-black dark:hover:text-white"
              data-testid="button-login-sidebar"
            >
              <LogIn className="h-5 w-5 flex-shrink-0 transition-transform duration-200 group-hover:scale-110 text-charcoal-500 dark:text-white/50 group-hover:text-black dark:group-hover:text-white" />
              <span>Login</span>
            </div>
          )
        )}
