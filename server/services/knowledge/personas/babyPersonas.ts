...
function generateBabyPersona(ethnicity: string, sex: 'Female' | 'Male', size: string, sizeIndex: number): Persona {
  ...
  const hairColor = sizeIndex < 3 ? 'Soft downy fuzz' : (['White', 'Diverse'].includes(ethnicity) ? 'Light brown' : 'Black');
  const hairStyle = sizeIndex < 2 ? 'Soft downy fuzz' : (sex === 'Female' ? 'Soft wisps with tiny bow' : 'Soft baby hair');

  return {
    id, name, age: 'Baby', sex, ethnicity: ethnicity as Ethnicity, size: size as Size,
    height: specs.height, weight: specs.weight, build: specs.build,
    facialFeatures: `Round baby face with chubby cheeks, big curious ${eyeColor.toLowerCase()} eyes, tiny button nose, gummy smile`,
    hairStyle, hairColor, eyeColor, skinTone,
    fullDescription: `${name} is a ${specs.ageDesc} baby ${sex.toLowerCase()} of ${ethnicity} heritage, measuring ${specs.height} and weighing approximately ${specs.weight}. ${sex === 'Female' ? 'She' : 'He'} has a ${specs.build} with adorable baby proportions including chubby limbs and rounded baby tummy. ${sex === 'Female' ? 'Her' : 'His'} round baby face has chubby cheeks, big curious ${eyeColor.toLowerCase()} eyes, a tiny button nose, and an adorable baby smile.`
  };
}

export const BABY_FEMALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as Record<Ethnicity, Record<Size, Persona[]>>;
export const BABY_MALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as Record<Ethnicity, Record<Size, Persona[]>>;

ETHNICITIES.forEach(ethnicity => {
  BABY_FEMALE_PERSONAS[ethnicity] = {} as Record<Size, Persona[]>;
  BABY_MALE_PERSONAS[ethnicity] = {} as Record<Size, Persona[]>;
...
export const BABY_PERSONAS = { female: BABY_FEMALE_PERSONAS, male: BABY_MALE_PERSONAS };
