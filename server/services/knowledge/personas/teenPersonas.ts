...
function generateTeenPersona(ethnicity: string, sex: 'Female' | 'Male', size: string, sizeIndex: number): Persona {
  ...
  return {
    ...
    fullDescription: `${name} is a ${13 + sizeIndex}-year-old teen ${sex.toLowerCase()} of ${ethnicity} heritage, standing ${specs.height} tall and weighing approximately ${specs.weight}. ${sex === 'Female' ? 'She' : 'He'} has a ${specs.build} with developing teen proportions. ${sex === 'Female' ? 'Her' : 'His'} face has clear youthful skin, bright ${eyeColor.toLowerCase()} eyes, and a fresh energetic appearance. ${sex === 'Female' ? 'Her' : 'His'} ${hairColor.toLowerCase()} hair is styled in a ${hairStyle.toLowerCase()} manner.`
  };
}

export const TEEN_FEMALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as Record<Ethnicity, Record<Size, Persona[]>>;
export const TEEN_MALE_PERSONAS: Record<Ethnicity, Record<Size, Persona[]>> = {} as Record<Ethnicity, Record<Size, Persona[]>>;

ETHNICITIES.forEach(ethnicity => {
  TEEN_FEMALE_PERSONAS[ethnicity] = {} as Record<Size, Persona[]>;
  TEEN_MALE_PERSONAS[ethnicity] = {} as Record<Size, Persona[]>;
